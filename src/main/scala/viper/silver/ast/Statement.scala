...
case class Seqn(ss: Seq[Stmt], locals: Seq[LocalVarDecl])(val pos: Position = NoPosition, val info: Info = NoInfo, val errT: ErrorTrafo = NoTrafos) extends Stmt with Scope {

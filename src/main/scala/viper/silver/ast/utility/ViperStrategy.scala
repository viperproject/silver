...
object ViperStrategy {
  ...
  def viperDuplicator: PartialFunction[(Node, Seq[Any], (Position, Info, ErrorTrafo)), Node] = {
    ...
    case (f: Function, Seq(parameters: Seq[LocalVarDecl@unchecked], aType: Type, preconditions: Seq[Exp@unchecked], decreases: Seq[Exp@unchecked], postconditions: Seq[Exp@unchecked], body: Option[Exp@unchecked]), meta) =>
      Function(f.name, parameters, aType, preconditions, decreases, postconditions, body)(meta._1, meta._2, meta._3)

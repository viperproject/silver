...
case class Add(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(AddOp)
case class Sub(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(SubOp)
case class Mul(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(MulOp)
case class Div(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(DivOp)
case class Mod(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(ModOp)

// Integer comparison expressions
case class LtCmp(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(LtOp)
case class LeCmp(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(LeOp)
case class GtCmp(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(GtOp)
case class GeCmp(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(GeOp)

// Equality and non-equality (defined for all types)
case class EqCmp(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends EqualityCmp("==")
case class NeCmp(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends EqualityCmp("!=")

/** Integer literal. */
case class IntLit(i: BigInt)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends Literal {
  lazy val typ = Int
}

/** Integer unary minus. */
case class Minus(exp: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainUnExp(NegOp)

// Boolean expressions
case class Or(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(OrOp) {
  Consistency.checkNoPositiveOnly(left)
  Consistency.checkNoPositiveOnly(right)
}
case class And(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(AndOp)

case class Implies(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(ImpliesOp) {
  Consistency.checkNoPositiveOnly(left)
}

case class MagicWand(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo)
...
/** Boolean negation. */
case class Not(exp: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainUnExp(NotOp) {
...
object BoolLit {
  ...
  def apply(b: Boolean)(pos: Position = NoPosition, info: Info = new NoInfo) = if (b) TrueLit()(pos, info) else FalseLit()(pos, info)
}
case class TrueLit()(val pos: Position = NoPosition, val info: Info = new NoInfo) extends BoolLit(true)
case class FalseLit()(val pos: Position = NoPosition, val info: Info = new NoInfo) extends BoolLit(false)

case class NullLit()(val pos: Position = NoPosition, val info: Info = new NoInfo) extends Literal {
  lazy val typ = Ref
}
...
case class FieldAccessPredicate(loc: FieldAccess, perm: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends AccessPredicate

/** An accessibility predicate for a predicate location. */
case class PredicateAccessPredicate(loc: PredicateAccess, perm: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends AccessPredicate
 ...
 */
case class InhaleExhaleExp(in: Exp, ex: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends Exp {
...
case class WildcardPerm()(val pos: Position = NoPosition, val info: Info = new NoInfo) extends PermExp

/** The full permission. */
case class FullPerm()(val pos: Position = NoPosition, val info: Info = new NoInfo) extends AbstractConcretePerm(1, 1)

/** No permission. */
case class NoPerm()(val pos: Position = NoPosition, val info: Info = new NoInfo) extends AbstractConcretePerm(0, 1)

/** An epsilon permission. */
case class EpsilonPerm()(val pos: Position = NoPosition, val info: Info = new NoInfo) extends PermExp

/** A concrete fraction as permission amount. */
case class FractionalPerm(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(FracOp) with PermExp
{
  require(left.typ==Int)
  require(right.typ==Int)
}

case class PermDiv(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(PermDivOp) with PermExp
{
  require(left.typ==Perm)
  require(right.typ==Int)
}
/** The permission currently held for a given location. */
case class CurrentPerm(loc: LocationAccess)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends PermExp

// Arithmetic expressions
case class PermMinus(exp: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainUnExp(PermNegOp) with PermExp
case class PermAdd(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(PermAddOp) with PermExp
case class PermSub(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(PermSubOp) with PermExp
case class PermMul(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(PermMulOp) with PermExp
case class IntPermMul(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(IntPermMulOp) with PermExp

// Comparison expressions
case class PermLtCmp(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(PermLtOp)
case class PermLeCmp(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(PermLeOp)
case class PermGtCmp(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(PermGtOp)
case class PermGeCmp(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends DomainBinExp(PermGeOp)
...
object FuncApp {
  def apply(func: Function, args: Seq[Exp])(pos: Position = NoPosition, info: Info = new NoInfo) : FuncApp = FuncApp(func.name, args)(pos,info,func.typ,func.formalArgs)
}
...
object DomainFuncApp {
  def apply(func : DomainFunc, args: Seq[Exp], typVarMap: Map[TypeVar, Type])(pos: Position = NoPosition, info: Info = new NoInfo) : DomainFuncApp =
...
object PredicateAccess {
  def apply(args: Seq[Exp], predicate:Predicate)(pos: Position = NoPosition, info: Info = new NoInfo) : PredicateAccess = PredicateAccess(args, predicate.name)(pos, info)
}

// --- Conditional expression

/** A conditional expressions. */
case class CondExp(cond: Exp, thn: Exp, els: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo)
...
case class Unfolding(acc: PredicateAccessPredicate, body: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends Exp {
...
case class UnfoldingGhostOp(acc: PredicateAccessPredicate, body: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends GhostOperation
case class FoldingGhostOp(acc: PredicateAccessPredicate, body: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends GhostOperation

case class ApplyingGhostOp(exp: Exp, body: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends GhostOperation {
  require(exp isSubtype Wand, s"Expected wand but found ${exp.typ} ($exp)")
}

case class PackagingGhostOp(wand: MagicWand, body: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends GhostOperation {
...
case class Old(exp: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends OldExp { Consistency.checkNoPositiveOnly(exp) }
case class ApplyOld(exp: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends OldExp { Consistency.checkNoPositiveOnly(exp) }
...
case class LabelledOld(exp: Exp, oldLabel: String)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends OldExp {
...
case class Let(variable: LocalVarDecl, exp: Exp, body: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends Exp {
...
case class Forall(variables: Seq[LocalVarDecl], triggers: Seq[Trigger], exp: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends QuantifiedExp {
...
case class Exists(variables: Seq[LocalVarDecl], exp: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends QuantifiedExp {
...
case class ForPerm(variable: LocalVarDecl, accessList: Seq[Location], body: Exp)
                  (val pos: Position = NoPosition, val info: Info = new NoInfo) extends Exp with QuantifiedExp {
...
case class Trigger(exps: Seq[Exp])(val pos: Position = NoPosition, val info: Info = new NoInfo) extends Node with Positioned with Infoed {
...
case class LocalVar(name: String)(val typ: Type, val pos: Position = NoPosition, val info: Info = new NoInfo) extends AbstractLocalVar with Lhs {
  require(Consistency.validUserDefinedIdentifier(name))
  require(typ != null)
}

/** A special local variable for the result of a function. */
case class Result()(val typ: Type, val pos: Position = NoPosition, val info: Info = new NoInfo) extends AbstractLocalVar {
...
case class EmptySeq(elemTyp: Type)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends SeqExp {
...
case class ExplicitSeq(elems: Seq[Exp])(val pos: Position = NoPosition, val info: Info = new NoInfo) extends SeqExp {
...
case class RangeSeq(low: Exp, high: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends SeqExp {
...
case class SeqAppend(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends SeqExp with PrettyBinaryExpression {
...
case class SeqIndex(s: Exp, idx: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends SeqExp {
...
case class SeqTake(s: Exp, n: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends SeqExp {
  require(s.typ.isInstanceOf[SeqType])
...
case class SeqDrop(s: Exp, n: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends SeqExp {
  require(s.typ.isInstanceOf[SeqType])
  require(n isSubtype Int)
  lazy val typ = s.typ
  def getArgs = Seq(s,n)
...
case class SeqContains(elem: Exp, s: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends SeqExp with PrettyBinaryExpression {
  require(s.typ.isInstanceOf[SeqType])
...
case class SeqUpdate(s: Exp, idx: Exp, elem: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends SeqExp {
  require(s.typ.isInstanceOf[SeqType])
  require(idx isSubtype Int)
  require(elem isSubtype s.typ.asInstanceOf[SeqType].elementType)
...
case class SeqLength(s: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends SeqExp {
  require(s.typ.isInstanceOf[SeqType])
  lazy val typ = Int
  def getArgs = Seq(s)
...
case class EmptySet(elemTyp: Type)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends SetExp {
...
case class ExplicitSet(elems: Seq[Exp])(val pos: Position = NoPosition, val info: Info = new NoInfo) extends SetExp {
  require(elems.nonEmpty)
  require(elems.tail.forall(e => e.typ == elems.head.typ))
  elems foreach Consistency.checkNoPositiveOnly
...
case class EmptyMultiset(elemTyp: Type)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends MultisetExp {
...
case class ExplicitMultiset(elems: Seq[Exp])(val pos: Position = NoPosition, val info: Info = new NoInfo) extends MultisetExp {
  require(elems.nonEmpty)
  require(elems.tail.forall(e => e.typ == elems.head.typ))
  elems foreach Consistency.checkNoPositiveOnly
...
case class AnySetUnion(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends AnySetBinExp {
  require(left.typ == right.typ)
...
case class AnySetIntersection(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends AnySetBinExp {
  require(left.typ == right.typ)
  require(left.typ.isInstanceOf[SetType] || left.typ.isInstanceOf[MultisetType])
  lazy val priority = 8
  lazy val fixity = Infix(LeftAssociative)
...
case class AnySetSubset(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends AnySetBinExp {
  require(left.typ == right.typ)
  require(left.typ.isInstanceOf[SetType] || left.typ.isInstanceOf[MultisetType])
  lazy val priority = 8
...
/** Set difference. */
case class AnySetMinus(left: Exp, right: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends AnySetBinExp {
  require(left.typ == right.typ)
  require(left.typ.isInstanceOf[SetType] || left.typ.isInstanceOf[MultisetType])
  lazy val priority = 8
  lazy val fixity = Infix(NonAssociative)
...
/** Is the element 'elem' contained in the sequence 'seq'? */
case class AnySetContains(elem: Exp, s: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends AnySetBinExp {
...
/** The length of a sequence. */
case class AnySetCardinality(s: Exp)(val pos: Position = NoPosition, val info: Info = new NoInfo) extends AnySetUnExp {

...
object ModelParser {
  ...
  def boolFuncDef[_: P]: P[MapEntry] = P(Start ~ alternatives ~ End)
    .map(options => MapEntry(options.map(lhs => lhs -> "true").toMap, "false"))

  def alternatives[_: P]: P[Seq[Seq[String]]] = P(singleAlternative | multipleAlternatives)

  def multipleAlternatives[_: P]: P[Seq[Seq[String]]] = P("(or" ~ boolOption.rep(1) ~ ")")
  ...
  def getApplication(s: String): Seq[String] = {

...
object Transformer {
  ...
  def transform[A <: PNode](node: A,
    ...
    def recurse(parent: PNode): PNode = {
      val newNode = parent match {
        ...
        case PComp(vars, filter, body, binary, unit) => PComp(vars map go, go(filter), go(body), go(binary), go(unit))
  ...
  def parseTreeDuplicator: PartialFunction[(PNode, Seq[Any]), PNode] = {
    ...
    case (_: PComp, Seq(vars: Seq[PFormalArgDecl@unchecked], filter: PExp, body: PFieldAccess, binary: PIdnUse, unit: PExp)) => PComp(vars, filter, body, binary, unit)

...
trait FastPositioned {
  /** TODO get rid of 'implicit def liftPos' of Translator.scala and make these methods private. */
...
sealed trait PNode extends FastPositioned with Product with Rewritable {
  ...
  def duplicate(children: scala.Seq[Any]): PNode = {
...
case class PDomainType(domain: PIdnUse, args: Seq[PType]) extends PGenericType {
  ...
  //if (kind==PDomainTypeKinds.Domain)
...
object POpApp {
  ...
  def pArgS(n: Int) = {
    require(n >= 0); "#T" + n.toString
  }

  def pResS = "#R"

  def pArg(n: Int) = {
    require(n >= 0); PTypeVar(pArgS(n))
  }
...
case class PCall(func: PIdnUse, args: Seq[PExp], typeAnnotated: Option[PType] = None) extends POpApp with PLocationAccess {
  ...
  var function: PAnyFunction = _
  var extfunction: PPredicate = _
...
case class PPredicateAccess(args: Seq[PExp], idnuse: PIdnUse) extends PLocationAccess {
  ...
  var predicate: PPredicate = _

  override def signatures = if (predicate == null) Set()
  else
...
sealed trait PBinder extends PExp {
  ...
  var _typeSubstitutions: Set[PTypeSubstitution] = _
...
case class PExists(vars: Seq[PFormalArgDecl], body: PExp) extends PQuantifier {
...
case class PNoPerm() extends PSimpleLiteral {
  typ = Perm
}

case class PFullPerm() extends PSimpleLiteral {
  typ = Perm
}

case class PWildcard() extends PSimpleLiteral {
  typ = Perm
}

case class PEpsilon() extends PSimpleLiteral {
  typ = Perm
}
...
case class PAxiom(idndef: PIdnDef, exp: PExp)(val domainName: PIdnUse) extends PScope with PGlobalDeclaration

//urij: this was not a declaration before - but the constructor of Program would complain on name clashes

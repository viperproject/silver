...
import viper.silver.ast.pretty.FastPrettyPrinter.{ContOps, char, group, showStmt, showType, showVars, ssep, text, toParenDoc}
...
case class ExistentialElim(varList: Seq[LocalVarDecl], exp: Exp)(val pos: Position = NoPosition, val info: Info = NoInfo, val errT: ErrorTrafo = NoTrafos) extends ExtensionStmt {
